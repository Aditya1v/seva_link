...
async function tryLlmPrioritization(input, fallback) {
  ...
  const timeout = setTimeout(() => controller.abort(), Number(process.env.AI_TIMEOUT_MS || 8000));
        ...
        messages: [
          {
            role: "system",
            content:
              "You structure community relief requests for an NGO dispatch dashboard. Return only JSON with title, category, urgency, priorityScore, summary, reason, keywords. urgency is critical, high, medium, or low. priorityScore is 1-100.",
          },
          {
            role: "user",
